...
- publish 대상 맵 경로
- readiness 요약 3줄(권장)
  - 실행 명령: `node scripts/jumpmap-check-split-repo-readiness.mjs --summary-lines`
...
```bash
node scripts/jumpmap-check-split-repo-readiness.mjs --summary-lines
node scripts/jumpmap-check-split-repo-readiness.mjs --strict --summary-lines
```
